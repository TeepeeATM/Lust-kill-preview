...
export function BagView(){
  ...
  if(items.length===0)return <div className="empty-state"><h1>YOUR BAG IS EMPTY.</h1><p>Take a look at the current collection.</p><Link className="primary-button inline" href="/shop">SHOP ALL PIECES</Link></div>;
  ...
  return <div className="bag-list"><h1>YOUR BAG.</h1>{items.map(item=><div className="bag-row" key={`${item.slug}-${item.size}-${item.color}`}><div><strong>{item.name}</strong><p>{item.color.toUpperCase()} / SIZE {item.size} / QTY {item.quantity}</p></div><button type="button" onClick={()=>remove(item.slug,item.size,item.color)}>REMOVE</button></div>)}{checkoutEnabled?<section className="checkout-panel" aria-labelledby="payment-heading"><p className="section-kicker">SECURE CHECKOUT</p><h2 id="payment-heading">READY TO ORDER?</h2><button className="primary-button" type="button" onClick={cardCheckout} disabled={loading}>{loading?"OPENING CHECKOUT…":"GO TO CHECKOUT →"}</button>{error&&<p className="checkout-error" role="alert">{error}</p>}<p className="payment-help">Stripe securely processes card payments. Other payment options may appear when available on your device.</p></section>:<section className="checkout-panel"><p className="section-kicker">CHECKOUT</p><h2>ORDERS ARE NOT OPEN YET.</h2><p className="payment-help">Follow @lustkills__ for the release date.</p><a className="primary-button" href="https://www.instagram.com/lustkills__/" target="_blank" rel="noreferrer">FOLLOW @LUSTKILLS__ →</a></section>}<button className="text-button" type="button" onClick={clear}>CLEAR BAG</button></div>
}
